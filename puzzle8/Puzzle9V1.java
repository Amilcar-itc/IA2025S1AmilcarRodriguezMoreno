...
class PuzzleXSolver {
	...
	private NodoPuzzle[] solucion2;
	private byte[][] estadoObjetivo;
	private ListaSimplePuzzle historial;
	...
	private /*NodoPuzzle[] /**/void/**/ resolver2(NodoPuzzle pivote) {
		if (pivote.getTablero().equals(estadoObjetivo))
			// return new NodoPuzzle[] {pivote};
			solucion2 = new NodoPuzzle[] {pivote};
		if (!existeAntes2(pivote, historial)) {
			agregarACola(pivote);
	...
	private boolean existeAntes2(NodoPuzzle estado) {
		ListaSimplePuzzle pivote = historial;
		while(pivote != null) {
			if (pivote.getNodo().esIgual(estado) &&
				pivote.getNodo().getProfundidad()<estado.getProfundidad())
				return true;
			pivote = pivote.getSiguiente();
		}
		return false;
	}
	private boolean existeAntes2(NodoPuzzle estado, ListaSimplePuzzle pivote) {
		if (pivote == null)
			return false;
		else if (pivote.getNodo().esIgual(estado) &&
				pivote.getNodo().getProfundidad() < estado.getProfundidad())
			return true;
		return existeAntes2(estado, pivote.getSiguiente());/**/
	}
	...
	/*NodoPuzzle[] getFamilia(NodoPuzzle[] hijos) {
		
		return solucion2;
	}/**/
	
	private void agregarACola(NodoPuzzle nodo) {
		if (historial==null)
			historial = new ListaSimplePuzzle(nodo);
		else
			historial.add(nodo);
	}
...
class NodoPuzzle {
	...
	void setSucesion(NodoPuzzle[] nuevaSucesion) {
		sucesion = (nuevaSucesion != null) ? nuevaSucesion.clone() : null;
	}
	NodoPuzzle[] getSucesion() {
		return (sucesion != null) ? sucesion.clone() : new NodoPuzzle[0];
	}
